...
//     const spreadSheet = rows.map((row) =>
//         row.split(" ").filter((x) => x !== "")
//     );

//     let numberSpreadSheet: number[][] = spreadSheet.map((row) => {
//         return row.map((number) => Number(number));
//     });

//     const answer = numberSpreadSheet.map((row: number[]) => {
//         return Math.max(...row) - Math.min(...row);
//     });

//     console.log(answer.reduce((a, b) => a + b));

//     const answer2: number[] = numberSpreadSheet.map((row: number[]) => {
//         for (let column of row) {
//             for (let columnCheck of row) {
//                 if (column % columnCheck === 0 && column !== columnCheck)
//                     return column / columnCheck;
//             }
//         }
//         return 0;
//     });
